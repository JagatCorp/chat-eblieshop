...
class MessagesController extends Controller
{
    ...
    public function deleteUserFromChannel(Request $request)
    {
        ...
        $user_id = $request->input('user_id'); // Mengambil user_id dari formulir

        // Mencari saluran berdasarkan ID
        $channel = Channel::findOrFail($channel_id);

        // Menghapus pengguna dari saluran
        $channel->users()->detach($user_id); // Menghapus pengguna dengan user_id yang benar

        // Mengirim pesan
        $message = Chatify::newMessage([
            'from_id' => Auth::user()->id,
            'to_channel_id' => $channel_id,
            'body' => Auth::user()->name . ' has deleted a member from the group',
            'attachment' => null,
        ]);
        ...
        // Redirect back
        return redirect()->back()->with('success', 'User has been deleted from the channel.');
    }

    // update only owner yang chat

    public function updateOwner(Request $request)
    {
        $user = Auth::user();
        // $channel = $user->channel; // Menggunakan asumsi relasi antara User dan Channel

        // Ambil ID saluran yang ingin diupdate
        $channel_id = $request->input('channel_id');

        // Mencari saluran berdasarkan ID
        $channel = Channel::findOrFail($channel_id);
        // return response()->json(['success', $channel]);

        if ($channel) {
            // Periksa apakah pengguna memiliki izin untuk mengupdate saluran
            if ($user->id == $channel->owner_id) {
                $channel->is_chat_disable = $request->input('is_chat_disable');
                $channel->save();
                ...
                $message = Chatify::newMessage([
                    'from_id' => Auth::user()->id,
                    'to_channel_id' => $channel_id,
                    'body' => Auth::user()->name . ' updated the chat settings from the group',
                    'attachment' => null,
                ]);
                ...
                // dd($request->input('is_chat_disable'));
                return redirect()->back()->with('success', 'Chat settings updated successfully.');
            } else {
                return redirect()->back()->with('error', 'You do not have permission to update this channel.');
            }
        } else {
    ...
    public function leaveGroupChat(Request $request)
    {
        ...
        // Dapatkan ID pemilik grup
        $owner_id = Channel::find($channel_id)->owner_id;

        // Periksa apakah pengguna yang membuat permintaan adalah pemilik grup
        if (Auth::id() != $owner_id) {
            return response()->json([
                'error' => true,
    ...
    public function addMemberToGroup(Request $request)
    {
        ...
        try {
            ...
            // send notification/message to the group about the new member
            $message = Chatify::newMessage([
                'from_id' => Auth::user()->id,
                'to_channel_id' => $channel_id,
                'body' => Auth::user()->name . ' has added a new member to the group',
                'attachment' => null,
            ]);
            $message->user_avatar = Auth::user()->avatar;
            $message->user_name = Auth::user()->name;
